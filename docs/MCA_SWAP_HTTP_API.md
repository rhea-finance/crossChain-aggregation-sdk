...
The following is the current product-level support list used by `multi-chain-lending` Trade. It includes 18 mainnets. The SDK and HTTP API use the same direct chain ID for `fromChain`, `toChain`, and `AssetRef.chain`. Use the "Token query alias" only when querying tokens.

| Chain | Type | Direct chain ID | Token query alias |
| --- | --- | --- | --- |
| Ethereum | EVM | `1` | `eth` |
...
| Zcash | Non-EVM | `zcash` | `zcash`; `zec` is also accepted |
...
The branch is determined by whether the destination is the bound NEAR account, not by `submissionMode` in the preview. SDK callers pass the same direct chain ID, `near`, without an additional namespace conversion.
